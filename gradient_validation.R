...
generalized_wishart_bkp <- function(E, S, tau, sigma, nu, validate=TRUE)
{
...
generalized_wishart <- function(E, S, tau, sigma, nu, validate=TRUE)
{
  ...
  #check that S is symmetric with 0 diagonal
  ...
  if (gradient || hessian || partial)
  {
    ...
    if (hessian || partial)
    {
      # Golub GH, Pereyra V. 1973. The Differentiation of Pseudo-Inverses and Nonlinear Least Squares Problems Whose Variables Separate. SIAM Journal on Numerical Analysis 10(2): 413-432
      ...
      if(partial)
      {
        ...
        goo <- function(par) {
          ...
          D        <- diag(eigSigW$values[-nrow(Sigma)])
          ginvSigW <- P %*% solve(D) %*% t(P)
          -nu/2 * SigInvW - nu/4 * SigInvW %*% S %*% t(SigInvW)
        }
